...
describe('installer symlink→copy fallback', () => {
  ...
  it('falls back to copy when symlink throws EPERM', async () => {
    ...
    // Pre-create .claude/ so installSkillForAgent's project-level skip-optimization
    // (skips symlinking for non-universal agents whose config dir is absent) doesn't
    // short-circuit. We need the symlink attempted so the EPERM→copy fallback runs.
    await fsp.mkdir(join(projectDir, '.claude'), { recursive: true });
